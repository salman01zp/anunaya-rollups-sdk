    ...
    F: Field,
{
    ...
    fn encode(data: &[F], parity_size: usize) -> Result<Vec<Self::Share>, ErasureCodeError> {
    ...
    fn decode(shares: &[Self::Share], data_size: usize) -> Result<Vec<F>, ErasureCodeError> {
        if shares.len() < data_size {
            return Err(ErasureCodeError::InsufficientSharesError(format!(
...
#[cfg(test)]
mod test {
    use crate::erasure_code::{ReedSolomonErasureCode, ReedSolomonErasureCodeShard};
    use crate::traits::ErasureCode;
